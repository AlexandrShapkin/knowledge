...
- В модели **[OSI](../../Сети%20и%20протоколы/Модели/OSI.md):** между уровнями 2 и 3 (канальный ↔ сетевой).
- В модели **[TCP-IP](../../Сети%20и%20протоколы/Модели/TCP-IP.md):** относится к **сетевому уровню (Network Layer)**, но выполняет функции для передачи на уровне канала.
...
- **[IP](IP.md)** - Использует ARP для нахождения MAC перед отправкой пакета
- **[Ethernet](Ethernet.md)** - Используется для физической передачи ARP-запросов/ответов
- **[ICMP](ICMP.md)** - Ошибки ICMP могут возникать, если ARP не удаётся выполнить
- **[DHCP](DHCP.md)** - Перед выдачей IP проверяет конфликт (Gratuitous ARP)
